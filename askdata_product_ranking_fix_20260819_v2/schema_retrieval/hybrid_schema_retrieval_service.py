...
class HybridSchemaRetrievalService:
    ...
    def retrieve(
        ...
        schema_hits = self._ensure_product_sales_schema_coverage(
            query=query,
    ...
    def _ensure_product_sales_schema_coverage(
        self,
        query: str,
        schema_hits: List[SchemaHit],
    ) -> List[SchemaHit]:
        """补齐产品销售额问题所需的维度、指标与关系端点。

        原有关键词抽取、双路召回、RRF 和 Rerank 流程保持不变。这里只在
        精排结果缺失必要端点时补入 ``products.product_name``（产品维度）
        和 ``order_items.line_amount``（产品销售额指标）。后续
        ``build_schema_graph`` 会基于这两个端点恢复表及外键关系。
        """
        normalized_query = query.strip().lower()
        product_terms = ("产品", "商品", "product", "item", "sku")
        sales_terms = ("销售额", "销售金额", "成交额", "gmv", "revenue", "sales amount")

        if not (
            any(term in normalized_query for term in product_terms)
            and any(term in normalized_query for term in sales_terms)
        ):
            return schema_hits
        ...
        if required_endpoints.issubset(existing):
            return schema_hits

        document_by_endpoint = {
            (document.column.table_name, document.column.column_name): document
            for document in self.documents
        }
        completed_hits = list(schema_hits)
        fallback_score = max((hit.score for hit in schema_hits), default=0.0)

        for endpoint in required_endpoints:
            if endpoint in existing:
                continue
            document = document_by_endpoint.get(endpoint)
            if document is None:
                continue
            completed_hits.append(
                SchemaHit(

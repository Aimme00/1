...
(
  ...
  输出目标: customers.region，line_amount合计为sales_amount
        ...
        product_sales_rank = self._parse_product_sales_rank(normalized)
        if product_sales_rank:
            direction, limit = product_sales_rank
            direction_label = "升序返回最低" if direction == "ASC" else "降序返回最高"
            return f"""步骤1：
(
  数据库: trade_db,
  ...
  操作指令: 先在orders表中筛选状态为completed的订单；再通过order_id关联order_items；然后通过product_id关联products并按product_name汇总line_amount；最后按商品销售额{direction_label}{limit}名,
  输出目标: products.product_name，line_amount合计为sales_amount
)"""
...
(
  ...
  输出目标: 缺失，无法生成明确输出目标
    ...
    @staticmethod
    def _parse_product_sales_rank(normalized_query: str) -> tuple[str, int] | None:
        """识别商品销售额 Top/Bottom N，并保留排序方向和数量。"""
        if not any(term in normalized_query for term in ("产品", "商品")):
            return None
        if not any(term in normalized_query for term in ("销售额", "销售金额", "成交额")):
            return None

        lowest_terms = ("最低", "最少", "倒数", "bottom")
        highest_terms = ("最高", "最多", "前", "top", "排名", "排行")
        if any(term in normalized_query for term in lowest_terms):
            direction = "ASC"
        elif any(term in normalized_query for term in highest_terms):
            direction = "DESC"
        else:
            return None

        matches = (
            re.search(r"(?:最低的?|最高的?|倒数|bottom|top|前)(\d+)", normalized_query),
            re.search(r"(\d+)个(?:产品|商品)", normalized_query),
        )
        limit = next((int(match.group(1)) for match in matches if match), 5)
        return direction, max(1, min(limit, 50))

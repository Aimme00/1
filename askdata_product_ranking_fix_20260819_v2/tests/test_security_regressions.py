...
from schema_retrieval import build_schema_graph
...
class SchemaAndAnswerRegressionTestCase(unittest.TestCase):
    ...
    def test_product_sales_schema_guard_restores_three_schema_layers(self) -> None:
        service = self.pipeline.schema_retrieval_service
        hits = service._ensure_product_sales_schema_coverage(
            query="销售额最低的5个产品是哪些",
            schema_hits=[],
        )
        ...
        self.assertTrue({"products", "order_items"}.issubset(graph.tables))
        self.assertIn(
            "product_name",
            {column.column_name for column in graph.columns["products"]},
        )
        self.assertIn(
            "line_amount",
            {column.column_name for column in graph.columns["order_items"]},
        )
        relation_endpoints = {
            frozenset(
                {
